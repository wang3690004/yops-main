...
export default {
  ...
  mounted() {
    const userInfo = Vue.ls.get("LOGIN_INFO");
    if (userInfo) {
      this.type = userInfo.type;
      this.form.setFieldsValue({
        username: userInfo.name,
        password: crypto.decrypt(userInfo.key),
      });
      this.remember = true;
    }
  },
  methods: {
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          loginParams = { ...values };
          loginParams._password = loginParams.password;
    ...
    doSubmit(loginParams) {
      ...
      this.Login(loginParams).then((res) => this.loginSuccess(res)).catch((err) => this.requestFailed(err));
    },
    loginSuccess(res) {
      if (res.ret >= 0) {
        this.setRemember();
    ...
    setRemember() {
      const { remember, username, _password, type } = this.loginInfo;
      if (remember) {
        const info = {
          name: username,
          key: crypto.encrypt(_password),
          type,
        }
        Vue.ls.set(`LOGIN_INFO`, info, 1000 * 60 * 60 * 24 * 30);
      } else {
        Vue.ls.set(`LOGIN_INFO`, '');
      }
    },
    requestFailed(err) {
